...
def _load_configs(base: Path) -> None:
    ...
    device_path = cfg / "device.yaml"
    if device_path.exists():
        with open(device_path) as f:
            configs["device"] = yaml.safe_load(f) or {}
    else:
        configs["device"] = {"mode": "auto"}
    ...
    cred_path = base / cred_rel if not Path(cred_rel).is_absolute() else Path(cred_rel)
...
def _select_model_runtime_path(model_path: str, device_mode: str, use_tensorrt: bool, label: str) -> str:
    """
    Select model file path based on runtime configuration without changing detector logic.
    - If mode == "cpu": force ONNX (.onnx)
    - If mode == "gpu": force TensorRT (.engine) -> if not available, log error and fallback to .onnx
    - If mode == "auto": detect GPU availability -> if GPU exists -> use TensorRT else use ONNX
    """
    p = Path(model_path)
    
    if device_mode == "cpu":
        onnx = p.with_suffix(".onnx")
        resolved = str(onnx) if onnx.exists() else model_path
        logger.info(f"[{label}] mode=cpu -> forced ONNX")
        return resolved

    if device_mode == "gpu":
        engine = p.with_suffix(".engine")
        if engine.exists():
            logger.info(f"[{label}] mode=gpu -> forced TensorRT")
            return str(engine)
        else:
            logger.error(f"[Error] GPU mode requested but TensorRT engine not found: {engine}. Falling back to ONNX.")
            onnx = p.with_suffix(".onnx")
            return str(onnx) if onnx.exists() else model_path

    # auto mode
    if use_tensorrt:
        engine = p.with_suffix(".engine")
        if engine.exists():
            logger.info(f"[{label}] mode=auto -> selected TensorRT (GPU)")
            return str(engine)

    # CPU-only or engine not found fallback
    onnx = p.with_suffix(".onnx")
    if onnx.exists():
        logger.info(f"[{label}] mode=auto -> selected ONNX (CPU)")
        return str(onnx)
        
    logger.info(f"[{label}] mode=auto -> selected fallback ({p.suffix})")
    return model_path
...
async def startup_event() -> None:
    ...
    try:
        ...
        device_mode = str(configs.get("device", {}).get("mode", "auto")).strip().lower()
        use_tensorrt = str(configs.get("box", {}).get("device", "cpu")).strip().lower() == "cuda"
        
        box_path = _select_model_runtime_path(box_path, device_mode, use_tensorrt, "Box Model")
        defect_path = _select_model_runtime_path(defect_path, device_mode, use_tensorrt, "Defect Model")
